...
func TestDiffWithRealGoCmp(t *testing.T) {
	...
	t.Run("SimpleTypes", func(t *testing.T) {
		testCases := []struct {
			...
			{name: "Complex", a: complex(1, 2), b: complex(3, 4)},
		}
		...
		for _, tc := range testCases {
			t.Run(tc.name, func(t *testing.T) {
				...
				if !strings.Contains(ourDiff, fmt.Sprintf("%v", tc.a)) || !strings.Contains(ourDiff, fmt.Sprintf("%v", tc.b)) {
					t.Errorf("Our diff doesn't contain the expected values: %v and %v", tc.a, tc.b)
				}
	...
	// Test with nil values and pointers
	t.Run("NilValuesAndPointers", func(t *testing.T) {
		type Node struct {
			Value int
			Next  *Node
		}
		...
		testCases := []struct {
			...
			{name: "NilVsValue", a: nil, b: 42},
			{name: "ValueVsNil", a: "hello", b: nil},
			{name: "NilPointerVsPointer", a: (*int)(nil), b: new(int)},
			{name: "PointerVsNilPointer", a: new(string), b: (*string)(nil)},
		}
		...
		// Test with cyclic structures
		n1 := &Node{Value: 1}
		n1.Next = n1 // Self-reference

		n2 := &Node{Value: 2}
		n2.Next = n2 // Self-reference
		...
		if !strings.Contains(ourDiff, "1") || !strings.Contains(ourDiff, "2") {
			t.Errorf("Our diff doesn't contain the expected values for cyclic structures")
		}
	})
	...
	t.Run("UnexportedFields", func(t *testing.T) {
		...
		// Check that our diff doesn't contain the unexported field
		if strings.Contains(ourDiff, "unexported") {
			t.Errorf("Our diff contains the unexported field which should be ignored")
		}
	})
