...
func (plugin *kubenetNetworkPlugin) ensureMasqRule() error {
	for _, currentCIDR := range plugin.nonMasqueradeCIDRs {
		if currentCIDR != zeroCIDRv4 && currentCIDR != zeroCIDRv6 {
			// 4 or 6
			isV6 := netutils.IsIPv6CIDRString(currentCIDR)
	...
	//TODO: khenidak
	// We are masquerading the secondry address for dual stack here
	...
	for idx, currentCIDR := range plugin.podCIDRs {
		if 0 < idx {
			...
			isV6 := netutils.IsIPv6CIDRString(cidrString)
			ipt := plugin.iptables
			if isV6 {
				ipt = plugin.iptablesv6
			}
...
// given a list of cidrs operated on by kubenet, does
// clear all address on bridge except those.
...
func (plugin *kubenetNetworkPlugin) setup(namespace string, name string, id kubecontainer.ContainerID, annotations map[string]string) error {
	...
	if err := plugin.addTrafficShaping(id, annotations); nil != err {
		return err
	}
