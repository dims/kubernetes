...
func NewContainerLogManager(runtimeService internalapi.RuntimeService, osInterface kubecontainer.OSInterface, maxSize string, maxFiles int, maxWorkers int, monitorDuration metav1.Duration) (ContainerLogManager, error) {
	if maxFiles <= 1 {
	...
	return &containerLogManager{
		...
		monitoringPeriod: monitorDuration,
	}, nil
}
...
func (c *containerLogManager) processQueueItems(worker int) {
	klog.V(4).InfoS("Starting container log rotate worker", "workerID", worker)
	for c.processContainer(context.Background(), worker) {
	}
	klog.V(4).InfoS("Terminating container log rotate worker", "workerID", worker)
}
...
func (c *containerLogManager) rotateLogs(ctx context.Context) error {
	...
	wait.PollInfinite(20*time.Millisecond, func() (done bool, err error) {
		return c.queue.Len() == 0, nil
	})
	klog.V(4).InfoS("Finished container log rotation sequence")
	return nil
}
