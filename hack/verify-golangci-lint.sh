...
# Install golangci-lint with the logcheck, kube-api-linter and sorted linters
# compiled in as module plugins (hack/tools/golangci-lint/main.go).
#
# hack/tools/golangci-lint is a separate Go module that pins the versions.
#
...
echo "installing golangci-lint with logcheck, kube-api-linter and sorted from hack/tools/golangci-lint into ${GOBIN}"
GOTOOLCHAIN="$(kube::golang::hack_tools_gotoolchain)" go -C "${KUBE_ROOT}/hack/tools/golangci-lint" install .
